...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        for (int i = 0; i < symbol.length; i++) {
            ...
            // First diagonal
            if (grid[0][0] == symbol[i] && grid[1][1] == symbol[i] && grid[2][2] == symbol[i]) {
                return message[i];
            }
            // Second diagonal
            if (grid[0][2] == symbol[i] && grid[1][1] == symbol[i] && grid[2][0] == symbol[i])
                return message[i];

            for (int j = 0; j < grid.length; j++) {
                // Check for rows
                if (grid[0][j] == symbol[i] && grid[1][j] == symbol[i] && grid[2][j] == symbol[i])
                    return message[i];

                // Check for columns
                if (grid[j][0] == symbol[i] && grid[j][1] == symbol[i] && grid[j][2] == symbol[i])
                    return message[i];
            }
